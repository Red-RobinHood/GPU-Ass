...
using namespace std;
#define MAX_L 128   // assuming L <= 128;
...
__global__ void gpu_sort(uint32_t *sequences, uint32_t *lengths, uint32_t *num_seq, int L) {
    // sort the sequences
    int row = blockIdx.x;
    if (row >= (int)(*num_seq)) return;

    int tid = threadIdx.x;
    int len = lengths[row];
    if (len <= 0) return;

    __shared__ uint32_t buf[MAX_L];

    int n = 1;
    while (n < len) n <<= 1;

    int base = row * MAX_L;
    if (tid < len) buf[tid] = sequences[base + tid];
    else if (tid < n) buf[tid] = 0xFFFFFFFFu;
    __syncthreads();

    for(int lar_n = 2; lar_n <= n ; lar_n <<= 1){
        #pragma unroll 8
        for(int break_pt = lar_n>>1; break_pt >= 1 ; break_pt >>= 1){
        int i = tid;      
        int j = i + break_pt;         
        bool is_pos = ((i & break_pt) == 0);
            if (is_pos && j < n) {                 
                bool is_asc = ((i & lar_n) == 0);
                uint32_t a = buf[i];
                uint32_t b = buf[j];
                if ((a > b) == is_asc) {
                    buf[i] = b;
    ...
    if (tid < len) sequences[base + tid] = buf[tid];
}
...
int main(int argc, char **argv) {
    ...
    // cout<<"Hi"<<endl;
    // uint32_t h_num_seq;
    // cudaMemcpy(&h_num_seq, num_seq, sizeof(uint32_t), cudaMemcpyDeviceToHost);
    // cout << "Total Sequences: " << h_num_seq << endl;
    

    dim3 grid_sort(30000);
    dim3 block_sort(MAX_L);
    gpu_sort<<<grid_sort, block_sort>>>(sequences, lengths, num_seq, L); //complete
